...
class deepmatcherdata(object):
    ...
    def __init__(self, 
        ...
        * For creating the non matching ('non match', left prod, right prod) tuples, the original ones
        ...
        self.__deeplabels = ['Left_' + key for key in keys] + ['Right_' + key for key in keys]
        self.matching = self.__getMatchingData(group_cols, keys)
        self.non_matching = None
        if create_non_match:
            self.non_matching = self.__getNonMatchingData(keys, perc)

    def splitDataTrainTest(df: pandas.DataFrame, train: float, test: float, val=None):
        if train <= 0 or train >= 1:
            raise Exception('Train percentage must be in (0, 1) interval')
        if train <= 0 or train >= 1:
            raise Exception('Train percentage must be in (0, 1) interval')
    ...
    def __getNonMatchingData(self, keys: list, perc=.75):
        """
    ...
    def __getMatchingData(self, group_cols, keys: list):
        """
