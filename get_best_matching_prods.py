...
if __name__ == "__main__":
    ...
    seller_offers = seller_offers[['brandSeller', 'nameSeller', 'descriptionSeller']].fillna(value='')
    ...
    ceneje_products = ceneje_products[['idProduct', 'brand', 'nameProduct']]
    ceneje_products['description'] = ''
    ceneje_products.columns = ['right_' + col for col in ceneje_products.columns]
    print(ceneje_products)

    unlabeled = pandas.DataFrame([
        chain.from_iterable([left_prod, right_prod])
        for row in seller_offers.iloc[:2, :].itertuples(index=False)
